...
def compute_waypoints(mav):
        ...
        wp_g[5] = xy2latlon(wp1, origin)
        wp_g[6] = xy2latlon(wp2, origin)
        wp_g[7] = xy2latlon(wp3, origin)
        wp_g[8] = xy2latlon(wp4, origin)
        wp_g[9] = xy2latlon(wp1, origin)
        wp_g[10] = xy2latlon(wp2, origin)
        wp_g[11] = xy2latlon(wp3, origin)
        wp_g[12] = xy2latlon(wp4, origin)
        wp_g[13] = xy2latlon(wp1, origin)
        wp_g[14] = xy2latlon(wp2, origin)
        wp_g[15] = xy2latlon(wp3, origin)
        wp_g[16] = xy2latlon(wp4, origin)
        ...
        for j in range(1,len(wps11)):
            ...
            wps11[j].x_lat = wp_g[j][0]
            wps11[j].y_long = wp_g[j][1]
            wps11[j].z_alt = wp_g[j][2]

        wps11[0] = wps11[1]
        wps11[16].command = 21
        wayp_16_l = wpMissionCnt()
        w = wayp_16_l.setWaypoints(3,21,False,True,0.0,0.0,0.0,float('nan'),wp_g[0][0], wp_g[0][1], wp_g[0][2])
        global wps3
        wps3.append(w)
        wps3.append(w)
